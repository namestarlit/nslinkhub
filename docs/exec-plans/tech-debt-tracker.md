...
| Email delivery | No email infrastructure; invitations/verification currently a logged no-op intent | Invitations/verification cannot reach users out-of-band | Direction set (`docs/design-docs/transactional-email.md`: Resend + React Email + outbox worker); build with the auth-delivery slice or nsauth work |
