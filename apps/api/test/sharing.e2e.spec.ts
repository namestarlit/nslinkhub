...
describe("Sharing, publication, and saves (e2e)", () => {
  ...
  const createCollection = async (bearer: string, slug: string, published = false) => {
    const res = await request(app.getHttpServer())
  ...
  it("link rotation cuts off previously valid tokens", async () => {
    ...
    await request(server).get(`/api/v1/collections/${id}/resources?s=${token}`).expect(200);
    ...
    await request(server).get(`/api/v1/collections/${id}/resources?s=${token}`).expect(404);
  });
  ...
  it("saves require publication and go dormant on unpublish", async () => {
    ...
    let saved = await request(server)
      ...
      .set("Authorization", `Bearer ${collaborator}`)
      .expect(200);
    let savedItem = (saved.body as { data: Array<{ id: string; available: boolean }> }).data.find(
      (c) => c.id === id,
    );
    ...
    saved = await request(server)
      ...
      .set("Authorization", `Bearer ${collaborator}`)
      .expect(200);
    savedItem = (saved.body as { data: Array<{ id: string; available: boolean }> }).data.find(
      (c) => c.id === id,
    );
